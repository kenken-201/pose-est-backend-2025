...
- [ ] **ルーター実装 (`POST /api/v1/process`)**
  - [ ] ファイル一時保存 (UploadFile -> TempFile)
  - [ ] 動画バリデーション (フォーマット、時間制限)
  - [ ] UseCase 呼び出し + 結果変換
  - [ ] ドメイン例外 -> API 例外変換
- [ ] **E2E テスト（`TestClient` 使用）**
  - [ ] 正常系 (短い動画ファイル)
  - [ ] 異常系 (無効フォーマット、時間超過、パラメータ不正)
  - [ ] 大規模動画のモックテスト
- [ ] **ミドルウェア設定**
  - [ ] CORS (Cloudflare Pages からのアクセス許可)
  - [ ] Request Logging (処理時間、ステータス)
  - [ ] ファイルサイズ制限
- [ ] **依存関係の環境変数対応**
  - [ ] `dependencies.py` の設定読み込みを環境変数化
