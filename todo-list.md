...
- [ ] MoveNet 推定エンジン設計
  - [ ] `MoveNetPoseEstimator` クラス設計 (Singleton, Multi-person)
  - [ ] スコア閾値フィルタリング戦略
- [ ] 動画処理パイプライン設計
  - [ ] `OpenCVVideoSource` 設計 (読み込み + メタデータ)
  - [ ] `OpenCVVideoSink` 設計 (フレーム書き出し)
  - [ ] `FFmpegAudioMerger` 設計 (音声結合)
- [ ] ストレージサービス設計
  - [ ] `R2StorageService` 設計 (Boto3/S3 互換)
  - [ ] `TempFileManager` 設計 (一時ファイル管理)
- [ ] **🛑 [Review] インフラ設計の承認**
